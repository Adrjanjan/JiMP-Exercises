...
TEST(factorial_test, factorial_of_0_is_1) {
  EXPECT_EQ(1, CountIntToIntFactorial(0));
}

TEST(factorial_test, factorial_of_5_is_120) {
  EXPECT_EQ(120, CountIntToIntFactorial(5));
}
...
TEST_P(FactorialDataDrivenTests, FactorialShouldReturnExpectedResult) {
  ...
  EXPECT_EQ(p.second, CountIntToIntFactorial(p.first)) << "Did call CountIntToIntFactorial (" << p.first << ")\n";
}
